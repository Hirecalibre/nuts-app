 ...
 * @param  {Boolean} opts.isRelease? : check if we are updating app used release endpoint
  ...
  return this.list().then((versions) => {
    return _.chain(versions)
      .filter((version) => {
        ...
        // Check tag request version
        return opts.tag === "latest" || opts.isRelease
          ? semver.gte(version.tag, opts.tag)
          : semver.satisfies(version.tag, opts.tag)
      })
      .value()
  })
}
