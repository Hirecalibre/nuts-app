...
const Nuts = function Nuts(opts) {
  ...
  // this.router.get(withPrefix(`download/:tag/:platform`), onDownload)
  ...
  this.router.get(
    withPrefix(`release/:tag/:filename`),
    this.onReleaseCheck.bind(this),
  )
...
Nuts.prototype.onReleaseCheck = function (req, res, next) {
  if (!req.params.tag) throw new Error('Requires "version" parameter')
  if (!req.params.filename) throw new Error('Requires "filename" parameter')

  const tag = req.params.tag
  const filename = req.params.filename
  const error_msg = `No download available for filename: "${_.escape(filename)}" with version: "${tag}"`;
...
Nuts.prototype.onHealthCheck = function (req, res, next) {
