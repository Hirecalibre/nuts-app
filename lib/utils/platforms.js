...
// Reduce a platform id to its type
...
const detectPlatform = (platform) => {
  ...
  // Detect prefix: osx, windows or linux
  ...
  if (
    ...
    if (_.includes(name, "arm")) {
      prefix = platforms.OSX_ARM
    } else {
...
const detectPlatformByUserAgent = (useragent) => {
  if (useragent.isMac) return platforms.OSX_64
  else if (
...
const resolveForVersion = (version, platformID, opts) => {
  ...
  return _.chain(version.platforms)
    ...
    .sort((p1, p2) => {
      ...
      // Order by file type if same architecture
      if (result == 0) {
...
const platforms = {
  ...
  OSX_64: "osx_64",
  OSX_ARM: "osx_arm",
  OSX_ARM32: "osx_arm_32",
  OSX_ARM64: "osx_arm_64",
  WINDOWS: "windows",
